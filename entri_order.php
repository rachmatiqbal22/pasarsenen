...
<div id="user-nav" class="navbar navbar-inverse">
  <ul class="nav">
    ...
    <li class=""><a title="" href="logout.php"><i class="icon icon-share-alt"></i> <span class="text">Keluar</span></a></li>
  </ul>
</div>
...
<div id="sidebar"><a href="entri_order.php" class="visible-phone"><i class="icon shopping-cart"></i> <span>Order</span></a>
  <ul>
    ...
    if($r['id_level'] == 1){
  ?>
    <li> <a href="beranda.php"><i class="icon icon-home"></i> <span>Data Pengguna</span></a> </li>
    ...
    <li> <a href="logout.php"><i class="icon icon-sign-out"></i> <span>Keluar</span></a> </li>
  <?php
        ...
        <div class="widget-box span12">
          ...
          <div class="widget-content">
            <div class="alert alert-info alert-block">
              ...
              Silahkan tunggu pesanan tiba di alamat saudara. Apabila pesana tiba, silahkan lakukan proses pembayaran!
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  $no_order_fiks = 1;
                  $query_order_fiks = "select * from tb_pesan natural join tb_sayur where id_user = $id and status_pesan != 'sudah'";
                ...
                <tr>
                  ...
                  <td><?php echo $r_order_fiks['nama_sayur'];?></td>
                ...
                <tr>
                  ...
                  <td class="right"></td>
                  <td class="right"></td>
                  <td class="right"><center><strong><?php echo $result_harga['alamat'];?></strong></center></td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <div class="row-fluid">
                ...
                <ul>
                  ...
                  <?php
                    ...
                    while($r_dt_pesan = mysqli_fetch_array($sql_lihat_pesan)){
                      array_push($pesan, $r_dt_pesan['id_sayur']);
                    }

                    $query_data_makanan = "select * from tb_sayur where stok > 0 order by id_sayur desc";
                    $sql_data_makanan = mysqli_query($conn, $query_data_makanan);
                    $no_makanan = 1;

                    while($r_dt_sayur = mysqli_fetch_array($sql_data_makanan)){
                  ?>
                      <li class="span2"> 
                        <a> <img src="gambar/<?php echo $r_dt_sayur['gambar_sayur']?>" alt="" > </a>
                        <div class="actions">
                          <a class="lightbox_trigger" href="gambar/<?php echo $r_dt_sayur['gambar_sayur'];?>"><i class="icon-search"></i>&nbsp;Lihat</a> 
                        </div>
                        <table class="table table-bordered">
                          <tbody>
                            <tr>
                              <td><?php echo $r_dt_sayur['nama_sayur'];?></td>
                            </tr>
                            <tr>
                              <td>Nama Pedagang</td>
                              <td><?php echo $r_dt_sayur['nama_pedagang'];?></td>
                            </tr>
                            <tr>
                              <td>Harga / Kg</td>
                              <td>Rp. <?php echo $r_dt_sayur['harga'];?>,-</td>
                            </tr>
                            <tr>
                              <td>Stok</td>
                              <td><?php echo $r_dt_sayur['stok'];?> Kg</td>
                            </tr>
                          </tbody>
                        </table>
                        <form action="" method="post">
                          <?php
                            if(in_array($r_dt_sayur['id_sayur'], $pesan)){
                          ?>
                              <button type="submit" value="<?php echo $r_dt_sayur['id_sayur'];?>" name="tambah_pesan" class="btn btn-danger btn-mini" disabled>
                                <i class='icon-shopping-cart'></i>&nbsp;&nbsp;Telah dipesan &nbsp;&nbsp;
                              </button>
                          <?php
                            } else {
                          ?>
                              <button type="submit" value="<?php echo $r_dt_sayur['id_sayur'];?>" name="tambah_pesan" class="btn btn-success btn-mini">
                    ...
                    <?php
                      ...
                      if(isset($_REQUEST['tambah_pesan'])){
                        ...
                        $id_sayur = $_REQUEST['tambah_pesan'];

                        $query_tambah_pesan = "insert into tb_pesan values('', '$id', '', '$id_sayur', '', '')";
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th style="width: 30%">Menu Pesanan</th>
              ...
              <tbody>
              <?php
                $query_draft_pesan = "select * from tb_pesan natural join tb_sayur where id_user = $id and status_pesan != 'sudah'";
                $sql_draft_pesan = mysqli_query($conn, $query_draft_pesan);

                while($r_draft_pesan = mysqli_fetch_array($sql_draft_pesan)){
              ?>
                <tr class="odd gradeX">
                  <td><span id="<?php echo "nama".$r_draft_pesan['id_pesan']; ?>"><?php echo $r_draft_pesan['nama_sayur'];?></span></td>
                  <input id="<?php echo "harga".$r_draft_pesan['id_pesan']; ?>" class="span8" type="hidden" value="<?php echo $r_draft_pesan['harga'];?>"/>
                  <td>
                    <center>
                      <input id="<?php echo "jumlah".$r_draft_pesan['id_pesan']; ?>" class="span8" name="jumlah<?php echo $r_draft_pesan['id_sayur']; ?>" type="number" value="" placeholder="" onchange="return operasi()"/>
                    </center>
                  </td>
                  <td>
                    <form action="" method="post">
              ...
              ?>
                <tr class="odd gradeX">
                  ...
                  <td>
                    <center>
                      <input class="span8" name="alamat" type="text" value="" placeholder="" />
                    </center>
                  </td>
                  <td>
                ...
                <tr class="odd gradeX">
                  ...
                  <td>
                    <center>
                      <input class="span8" name="no_hp" type="text" value="" placeholder="" />
                    </center>
                  </td>
                  <td>
      ...
      <?php
          ...
          if(isset($_POST['proses_pesan'])){
            ...
            $time = Date('YmdHis');
            echo $time;
            $query_simpan_order = "insert into tb_order values('', '$id_admin', '$id_pengunjung', $time, '$alamat', '$total_harga', '$uang_bayar', '$uang_kembali', '$status_order')";
            ...
            // Query untuk mengambil data dari tb_pesan dan tb_sayur
$query_ubah_jumlah = "SELECT * FROM tb_pesan LEFT JOIN tb_sayur ON tb_pesan.id_sayur = tb_sayur.id_sayur WHERE id_user = ? AND status_pesan != 'sudah'";
$stmt = mysqli_prepare($conn, $query_ubah_jumlah);
mysqli_stmt_bind_param($stmt, 'i', $id);
mysqli_stmt_execute($stmt);
$result_ubah_jumlah = mysqli_stmt_get_result($stmt);

while($r_ubah_jumlah = mysqli_fetch_array($result_ubah_jumlah)) {
    $tahu = $r_ubah_jumlah['id_sayur'];
    $tempe = isset($_POST['jumlah'.$tahu]) ? $_POST['jumlah'.$tahu] : 0;

    // Pastikan $tempe adalah integer
    $tempe = intval($tempe);

    $id_pesan = $r_ubah_jumlah['id_pesan'];
    
    // Query untuk mengambil stok dari tb_sayur
    $query_stok = "SELECT stok FROM tb_sayur WHERE id_sayur = ?";
    $stmt_stok = mysqli_prepare($conn, $query_stok);
    mysqli_stmt_bind_param($stmt_stok, 'i', $tahu);
    mysqli_stmt_execute($stmt_stok);
    $result_stok = mysqli_stmt_get_result($stmt_stok);
    $stok_data = mysqli_fetch_array($result_stok);

    // Pastikan stok adalah integer
    $stok = isset($stok_data['stok']) ? intval($stok_data['stok']) : 0;

    // Hitung sisa stok
    $sisa_stok = $stok - $tempe;

    // Update tb_pesan
    $query_proses_ubah = "UPDATE tb_pesan SET jumlah = ?, id_order = ? WHERE id_sayur = ? AND id_user = ? AND status_pesan != 'sudah'";
    $stmt_proses_ubah = mysqli_prepare($conn, $query_proses_ubah);
    mysqli_stmt_bind_param($stmt_proses_ubah, 'iiii', $tempe, $id_ordernya, $tahu, $id);
    mysqli_stmt_execute($stmt_proses_ubah);

    // Update tb_sayur
    $query_kurangi_stok = "UPDATE tb_sayur SET stok = ? WHERE id_sayur = ?";
    $stmt_kurangi_stok = mysqli_prepare($conn, $query_kurangi_stok);
    mysqli_stmt_bind_param($stmt_kurangi_stok, 'ii', $sisa_stok, $tahu);
